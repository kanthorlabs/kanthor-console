...
import { useApps } from "../../hooks/application";

export const EndpointList: React.FC<IResourceComponentsProps> = () => {
  ...
  const { isLoading, apps } = useApps(appIds);
    ...
    <List>
      ...
      <Table {...tableProps} rowKey="id" loading={isLoading}>
        <Table.Column
          ...
          title={"Application"}
          render={(appId: string) => {
            return <TextField value={apps[appId]?.name} />;
          }}
        />
